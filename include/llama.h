...
extern "C" {
    ...
    struct llama_model_params {
        ...
        // dyn-ex: dynamic expert offloading from a pre-built .bin file
        const char * dyn_ex_path;       // path to VLLM\x02 .bin file (NULL = disabled)
        int32_t      dyn_ex_n_slots;    // number of GPU slots per layer (0 = disabled, must be power of 2)

        // reserved for future dyn-ex features
        const char * dyn_ex_predictor;  // path to predictor weights (NULL = no predictor)
    };
