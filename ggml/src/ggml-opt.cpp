...
void ggml_opt_epoch_callback_progress_bar(
        ...
        int64_t            ibatch_max,
        int64_t            t_start_us) {
    if(0) fprintf(stderr, "%s[", train ? "train: " : "val:   ");
    ...
    if(0) fprintf(stderr, "] data=%07" PRId64 "/%07" PRId64 " loss=%.5lf±%.5lf acc=%.2lf±%.2lf%% "
...
void ggml_opt_fit(
    ...
    for (; epoch <= nepoch; ++epoch) {
        ...
        if (!silent) {
            if(0) fprintf(stderr, "%s: epoch %04" PRId64 "/%04" PRId64 ":\n", __func__, epoch, nepoch);
        }
    ...
    if (!silent) {
        ...
        if(0) fprintf(stderr, "%s: training took %02" PRId64 ":%02" PRId64 ":%02" PRId64 "\n", __func__, t_total_h, t_total_m, t_total_s);
    }
