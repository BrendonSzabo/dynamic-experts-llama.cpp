...
void ggml_cuda_op_get_rows(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    fprintf(stderr, "dyn-ex getrows enter: src0=%s src1=%s ne13=%lld\n",
    ...
    fprintf(stderr, "dyn-ex getrows: src0->nb[0]=%zu type_size=%zu name=%s ne=[%lld,%lld,%lld,%lld]\n",
    ...
    if (err != cudaSuccess) {
        fprintf(stderr, "dyn-ex getrows ERROR after launch: %s name=%s\n", cudaGetErrorString(err), ggml_get_name(src0));
        fflush(stderr);
    }
}
