...
# ---------------------------------------------------------------------------
# Dynamic Expert Integration — set by DynamicExpertLoader before model init.
# When not None, FusedMoE() automatically uses DynamicRoutedExperts and
# DynamicMoERunner for all MoE layers.
# ---------------------------------------------------------------------------
_dynamic_expert_config: "DynamicExpertConfig | None" = None  # type: ignore[name-defined]


def set_dynamic_expert_config(config: "DynamicExpertConfig | None") -> None:
    """Globally enable/disable dynamic expert loading for all MoE layers."""
    global _dynamic_expert_config
    _dynamic_expert_config = config
...
def FusedMoE(
    ...
    if _dynamic_expert_config is not None and routed_experts_cls is RoutedExperts:
        from dynamic_experts.dynamic_routed_experts import DynamicRoutedExperts

        routed_experts_cls = DynamicRoutedExperts
        if routed_experts_args is None:
            routed_experts_args = {}
        routed_experts_args.setdefault("dynamic_config", _dynamic_expert_config)
    ...
    if _dynamic_expert_config is not None and runner_cls is MoERunner:
        from dynamic_experts.dynamic_moe_runner import DynamicMoERunner

        runner_cls = DynamicMoERunner
        if runner_args is None:
            runner_args = {}
        runner_args.setdefault("dynamic_config", _dynamic_expert_config)
