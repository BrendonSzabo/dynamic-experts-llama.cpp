...
static bool dyn_ex_eval_callback(ggml_tensor * t, bool pre, void * user_data) {
    ...
    // write slot IDs into pre-allocated GPU tensor for MUL_MAT_ID
    if (il >= 0 && (size_t)il < de->t_slot_ids.size() && de->t_slot_ids[il]) {
        ...
        ggml_backend_tensor_set(de->t_slot_ids[il], slots.data(), 0, n_e * sizeof(int32_t));
    }
    ...
    if (il == 0 && n_e > 0) {
        ...
        if (sm && sm->data && gate_t && gate_t->data) {
            for (int i = 0; i < n_e; i++) {
                int32_t slot_idx = -2;
                ggml_backend_tensor_get(sm, &slot_idx, ids[i] * sizeof(int32_t), sizeof(int32_t));
                if (slot_idx >= 0) {
...
llm_graph_result * llama_context::process_ubatch(const llama_ubatch & ubatch, llm_graph_type gtype, llama_memory_context_i * mctx, ggml_status & ret) {
    ...
    if (!graph_reuse_disable && res->can_reuse(gparams) && !model.has_dyn_ex()) {
        ...
        // verify slot buffers unchanged after alloc
        for (int il = 0; il < (int)slot_saved.size(); il++) {
            auto * t = model.layers[il].ffn_gate_exps;
            if (!t) continue;
        ...
        // also check slot_map
        for (int il = 0; il < model.hparams.n_layer_all; il++) {
            auto * t = model.layers[il].ffn_slot_map;
...
llm_graph_params llama_context::graph_params(
    ...
    if(0) fprintf(stderr, "graph_params: dyn_ex_barrier=%p has_dyn=%d\n",
