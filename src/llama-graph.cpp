...
ggml_tensor * llm_graph_context::build_moe_ffn(
    ...
    // barrier: GPU writes expert IDs, CPU loads weights, writes slot IDs to t_slot_ids
...
<<<<<<< Updated upstream
    ...
    if (slot_map != nullptr) {
        ...
        //     (long long)selected_experts_slots->ne[0], (long long)selected_experts_slots->ne[1]);
=======
    // use pre-allocated slot IDs tensor from dyn_ex cache (written by barrier callback)
...
>>>>>>> Stashed changes
    ...
    if (gate_up_exps) {
        ...
        up = build_lora_mm_id(up_exps, cur, selected_experts_slots, up_exps_s); // [n_ff, n_expert_used, n_tokens]
        cb(up, "ffn_moe_up", il);
        ...
        if (gate_exps) {
            cur = build_lora_mm_id(gate_exps, cur, selected_experts_slots, gate_exps_s); // [n_ff, n_expert_used, n_tokens]
            cb(cur, "ffn_moe_gate", il);
        } else {
            cur = up;
        }
    ...
    experts = build_lora_mm_id(down_exps, cur, selected_experts_slots, down_exps_s); // [n_embd, n_expert_used, n_tokens]
